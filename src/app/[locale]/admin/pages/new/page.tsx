...
export default function NewPagePage() {
  ...
  const [template, setTemplate] = useState<'standard' | 'location'>('standard');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/admin/pages', {
        ...
        body: JSON.stringify({ title, content, template, seoTitle, metaDescription, featuredImage, ctaTitle, ctaButton, ctaLink, exploreTitle, exploreSubtitle, exploreDescription, exploreItems, faq, isPublished })
      });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 md:p-10">
      <h1 className="text-2xl font-bold text-[#0B1B40] mb-6">Create Page</h1>
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white border border-gray-200 rounded-3xl p-6 space-y-4">
          ...
          <div>
            ...
            <select value={template} onChange={e => setTemplate(e.target.value as any)} className="w-full border border-gray-300 rounded-full px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#1F51C6]">
              <option value="standard">Standard (Rich Text)</option>
              <option value="location">Location (Structured Template)</option>
            </select>
          </div>
          <div>
        ...
        {template === 'location' && (
          <div className="bg-white border border-gray-200 rounded-3xl p-6 space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Title</label>
              <input value={exploreTitle} onChange={e => setExploreTitle(e.target.value)} className="w-full border border-gray-300 rounded-full px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#1F51C6]" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Subtitle</label>
              <input value={exploreSubtitle} onChange={e => setExploreSubtitle(e.target.value)} className="w-full border border-gray-300 rounded-full px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#1F51C6]" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Description</label>
              <textarea value={exploreDescription} onChange={e => setExploreDescription(e.target.value)} className="w-full border border-gray-300 rounded-2xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#1F51C6] h-20" />
            </div>
            <div>
        ...
        <div className="bg-white border border-gray-200 rounded-3xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={ctaButton} onChange={e => setCtaButton(e.target.value)} className="w-full border border-gray-300 rounded-full px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#1F51C6]" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Link</label>
              <input value={ctaLink} onChange={e => setCtaLink(e.target.value)} className="w-full border border-gray-300 rounded-full px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#1F51C6]" />
            </div>
          </div>
        </div>
