...
export default function ExploreSection({ title, description, subtitle, items = [] }: ExploreSectionProps) {
  ...
  return (
    <section className="px-4 sm:px-6 lg:px-8 py-16 sm:py-20 bg-[#191e3b]">
      <div className="mb-12">
        {subtitle && (
          <p className="text-sm font-semibold uppercase tracking-wider text-[#77e1fb] mb-3">{subtitle}</p>
        )}
        <h2 className="text-3xl sm:text-4xl font-bold text-white" style={{ fontFamily: 'Radio Canada, sans-serif' }}>
          {title}
        </h2>
        {description && (
          <p className="text-white/60 mt-4 text-sm sm:text-base leading-relaxed max-w-4xl">{description}</p>
        )}
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
        {items.map((item, i) => (
          <div key={i} className="bg-white p-8 sm:p-10 rounded-[2rem] border border-white/10 hover:border-[#77e1fb]/30 transition-colors">
